...
import type { CreateArgv } from './create.js';
import command from './create.js'
import type { Arguments } from 'yargs'
import yargs from 'yargs'
import { makeMarkdownSample, makePieceMock, makeContext } from '../utils/context.fixtures.js'
...
describe('lib/commands/create', () => {
	...
	test('run', async () => {
		...
		spies.setFields = vi.spyOn(Piece, 'setFields').mockResolvedValueOnce(markdown)
	...
	test('run and set fields with json', async () => {
		...
		spies.pieceCreate = vi.spyOn(Piece, 'create').mockResolvedValueOnce(book)
		spies.pieceWrite = vi.spyOn(Piece, 'write').mockResolvedValueOnce()
		spies.pieceSetFields = vi.spyOn(Piece, 'setFields').mockResolvedValueOnce(book)
