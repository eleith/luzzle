...
function validateConfig(config, validator, ajvInstance, configName, suppressSuccessMessage = false) {
    if (!validator(config)) {
        console.error(`\n❌ ${configName} is invalid:`);
        validator.errors.forEach(err => {
            console.error(`  - Field: ${err.instancePath || '/'}${(err.keyword === 'required' && err.params && err.params.missingProperty) ? ` (missing: ${err.params.missingProperty})` : ''}`);
...
function runConfigCheck() {
    ...
    // --- Core Logic ---
    if (customUserConfigPath) { // If --file is present
        // Scenario: Check merged config with a specified file
        // Always validate default config, but suppress its success message
        if (!validateConfig(defaultConfig, validator, ajv, 'Default configuration (config.defaults.yaml)', true)) {
            hasErrors = true;
        }
        ...
        if (userConfig) {
            ...
            if (!validateConfig(mergedConfig, validator, ajv, `Merged configuration (config.defaults.yaml + ${actualUserConfigPath})`)) {
                hasErrors = true;
            }
        } else {
    ...
    } else { // If no --file is present (implicitly check default)
        // Scenario: Check default and suggest --file
        if (!validateConfig(defaultConfig, validator, ajv, 'Default configuration (config.defaults.yaml)')) {
            hasErrors = true;
        }
