<script lang="ts">
	...
	type PreviewStatus = 'idle' | 'enqueued' | 'running' | 'completed' | 'failed'
	...
	let assembled = $state<AssembledPreview | null>(null)
	...
	function startWatching(id: string | number) {
		...
		eventSource = new EventSource(`/api/preview/${id}/stream`)
		...
		eventSource.addEventListener('done', (e) => {
			const d = JSON.parse(e.data)
			if (d.state === 'completed' && d.result) {
				try {
					assembled = d.result as AssembledPreview
					status = 'completed'
				} catch (err) {
					status = 'failed'
				...
				if (d.errors && d.errors.length > 0) {
					errorMessage = d.errors[0]?.message || 'Unknown error'
				}
			}
			eventSource?.close()
			eventSource = null
		...
		eventSource.addEventListener('error', (e) => {
			...
			if (msgEvent.data) {
				try {
					...
					errorMessage = d.message || 'Stream error'
				} catch {
...
{#if status === 'completed' && assembled}
	<div class="preview-banner">
		<div class="preview-banner-left">
			<span class="preview-label">preview</span>
			<span class="preview-title">{assembled.piece.title}</span>
		</div>
	...
	<section class="preview-status">
		...
		<div class="status-card">
			...
			<div class="status-text">
				{#if status === 'idle'}
					<div class="status-title">Starting preview…</div>
					<div class="status-sub">Connecting to worker</div>
				{:else if status === 'enqueued'}
