...
async function parseArgs(args: string[]) {
	  const commands = [configCommand, themeCommand, syncCommand, transformCommand, backfillCommand]
	  ...
	  commands
	    ...
	    .demandCommand(
	      1,
	      'You need to specify a command [config, theme, sync, transform, backfill]'
	    )		.help()
		.showHelpOnFail(true)
		.parseAsync()
}
