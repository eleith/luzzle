...
For the builder to function correctly, the following paths **must** be mounted as volumes in the container:
...
*   `/app/assets`: The directory where optimized assets and OG images are generated (should be the Web Explorer's static assets folder).
*   `/app/archive`: The source directory containing your Markdown pieces and raw assets (as defined in your config).
...
* `LUZZLE_BUILD_TOKEN`: (Required) A secret token to authenticate webhook requests.
