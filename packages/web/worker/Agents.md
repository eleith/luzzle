...
* **Dependencies**: depends only on `@luzzle/core` and `@luzzle/web.config` (plus the heavy worker-only deps: `sharp`, `puppeteer`, `sidequest`). No `@luzzle/web.utils` or `@luzzle/web.tools` dependency.
