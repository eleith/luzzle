...
/* c8 ignore next 10 */
function getTagsPrompt(title: string, author: string, isbn?: string): string {
	const fullIsbn = isbn ? ` , ISBN: ${isbn}` : ''
	const statement = `${title} by ${author}${fullIsbn}`

	return OPENAI_USER_TAGS_PROMPT_STRUCTURE.replace('%s', statement)
}

/* c8 ignore next 10 */
function getDescPrompt(title: string, author: string, isbn?: string): string {
	const fullIsbn = isbn ? ` , ISBN: ${isbn}` : ''
	const statement = `${title} by ${author}${fullIsbn}`
...
async function generateTags(
	...
	const response = await openai.createChatCompletion({
		...
		messages: [
			...
			{ role: 'user', content: getTagsPrompt(title, author, isbn) },
		],
	})
...
async function generateDescription(
	...
	const response = await openai.createChatCompletion({
		...
		messages: [
			...
			{ role: 'user', content: getDescPrompt(title, author, isbn) },
		],
	})
