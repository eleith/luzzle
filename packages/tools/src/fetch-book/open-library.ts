...
async function search(title: string, author: string): Promise<Array<OpenLibrarySearchWork>> {
	try {
		...
		console.warn('open-library', `search returned http: ${response.statusCode}`)
	} catch (e) {
		console.error('open-library', e as string)
	}
...
async function findWork(id: string): Promise<OpenLibrarySearchWork | null> {
	try {
		...
		console.warn('open-library', `findWork returned http: ${response.statusCode}`)
	} catch (e) {
		console.error('open-library', e as string)
	}
...
async function getWork(workId: string): Promise<OpenLibraryWork | null> {
	try {
		...
		console.warn('open-library', `getWork returned http: ${response.statusCode}`)
	} catch (e) {
		console.error('open-library', e as string)
	}
...
const getBook = async (bookId: string): Promise<OpenLibraryBook | null> => {
	try {
		...
		console.warn('open-library', `getBook returned http: ${response.statusCode}`)
	} catch (e) {
		console.error('open-library', e as string)
	}
