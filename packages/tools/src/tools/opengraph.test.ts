...
vi.mock('fs/promises');
vi.mock('./opengraph/html.js');
vi.mock('./opengraph/png.js');
vi.mock('./opengraph/browser.js');
vi.mock('@luzzle/cli');
vi.mock('@luzzle/core');
...
describe('tools/opengraph', () => {
	...
	test('should generate an opengraph image for a single item', async () => {
		...
		await generateOpenGraphs('test', 'test', 'test', 'test', {});
	...
	test('should force generation', async () => {
		...
		await generateOpenGraphs('test', 'test', 'test', 'test', { force: true });
	...
	test('should limit generation', async () => {
		const { db, queries } = mockKysely();
		const browser = { close: vi.fn() };
		mocks.loadConfig.mockReturnValue({ paths: { database: 'test' } } as AppConfig);
		mocks.getDatabaseClient.mockReturnValue(db);
		...
		await generateOpenGraphs('test', 'test', 'test', 'test', { limit: 1 });
