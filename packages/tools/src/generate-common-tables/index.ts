...
async function dropWebTables(db: LuzzleDatabase): Promise<void> {
	await db.schema.dropTable('web_pieces').execute()
	await db.schema.dropTable('web_pieces_fts5').execute()
}
