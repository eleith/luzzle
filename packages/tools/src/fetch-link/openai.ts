...
function getTagsPrompt(url: string): string {
	return OPENAI_USER_TAGS_PROMPT_STRUCTURE.replace('%s', url)
}

function getSummaryPrompt(url: string): string {
	return OPENAI_USER_SUMMARY_PROMPT_STRUCTURE.replace('%s', url)
}

function getClassificationPrompt(url: string): string {
	return OPENAI_USER_CLASSIFY_PROMPT_STRUCTURE.replace('%s', url)
}

async function generateTags(apiKey: string, url: string): Promise<string[]> {
	...
	const response = await openai.createChatCompletion({
		...
		messages: [
			...
			{ role: 'user', content: getTagsPrompt(url) },
		],
	})
...
async function generateSummary(apiKey: string, url: string): Promise<string> {
	const openai = getClient(apiKey)
	...
	const response = await openai.createChatCompletion({
		...
		messages: [
			...
			{ role: 'user', content: getSummaryPrompt(url) },
		],
	})
...
async function generateClassification(
	...
	const response = await openai.createChatCompletion({
		...
		messages: [
			...
			{ role: 'user', content: getClassificationPrompt(url) },
		],
	})
