...
async function parseArgs(args: string[]) {
	const commands = [assetCommand, openGraphCommand, sqliteCommand, validateCommand, themeCommand]
	...
	commands
		...
		.demandCommand(
			1,
			'You need to specify a command [assets, opengraph, sqlite, validate, theme]'
		)
		.help()
		.showHelpOnFail(true)
		.parseAsync()
