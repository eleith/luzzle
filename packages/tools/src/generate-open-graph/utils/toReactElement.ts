// MIT License: https://github.com/etherCorps/svelte-h2j?tab=readme-ov-file#license
// function from https://github.com/etherCorps/svelte-h2j/blob/svelte-v5/src/lib/toReactElement.ts
...
function extractStyles(cssAst: NonNullable<AST.Root['css']>) {
...
function toReactElement(htmlString: string): VNode {
	...
	if (svelteAST && svelteAST.css) {
		styles = extractStyles(svelteAST.css as unknown as NonNullable<AST.Root['css']>)
	}

	walk(svelteAST as any, {
		enter(node: any, parent: any, _prop: any, index: any) {
			let newNode: any = {}
				...
				if (node.attributes.length > 0) {
					...
					node.attributes.forEach((attribute: any) => {
						if (Object.keys(styles).length && attribute.name === 'class') {
							...
							const originalClasses = attribute.value[0].data
								.split(' ')
								.filter((className: string) => !className.startsWith('svelte'))
