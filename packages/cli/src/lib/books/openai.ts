...
/* c8 ignore next 10 */
function getTagsPrompt(bookMd: BookMd): string {
...
/* c8 ignore next 10 */
function getDescPrompt(bookMd: BookMd): string {
  const { title, isbn, description, author, subtitle, coauthors } = bookMd.frontmatter
  const fullTitle = subtitle ? `"${title}: ${subtitle}"` : `"${title}"`
  const fullAuthor = coauthors ? `${author} and ${coauthors}` : author
  const fullIsbn = isbn ? ` , ISBN: ${isbn}` : ''
...
async function generateTags(apiKey: string, bookMd: BookMd): Promise<string[]> {
...
async function generateDescription(apiKey: string, bookMd: BookMd): Promise<string> {
  const openai = getClient(apiKey)
